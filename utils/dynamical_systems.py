from scipy.integrate import solve_ivp
import numpy as np

def iter_rk4(prev, t, h, f, fargs=None):
    """
    Runge-Kutta 4th order (vectorized).
    prev: (batch, n_dim)
    f: function f(t, y) that works with (batch, n_dim)
    """
    if fargs is None:
        k1 = f(t, prev)                         # (batch, n_dim)
        k2 = f(t + 0.5*h, prev + 0.5*h*k1)
        k3 = f(t + 0.5*h, prev + 0.5*h*k2)
        k4 = f(t + h, prev + h*k3)
    else:
        k1 = f(t, prev, fargs)
        k2 = f(t + 0.5*h, prev + 0.5*h*k1, fargs)
        k3 = f(t + 0.5*h, prev + 0.5*h*k2, fargs)
        k4 = f(t + h, prev + h*k3, fargs)

    return prev + (h/6.0)*(k1 + 2*k2 + 2*k3 + k4)  # (batch, n_dim)

class DS():
    def __init__(self, du_dt, n_dim, step, method='RK4'):
        self.du_dt = du_dt
        self.n_dim = n_dim
        self.step = step
        self.method = method

    def integrate(self, y0, T = 1, t0 = 0):
        """
        Integrates forward over a batch for T time steps
        y0 : (batch, n_dim)
        """
        batch = y0.shape[0]
        y_sol = np.zeros((batch, T, self.n_dim))
        y_step = y0 # (batch, n_dim)
        t_step = t0
        for t in range(T):
            if self.method == 'RK4':
                y_step = iter_rk4(y_step, t0, self.step, self.du_dt)
            else:
                # NOTE: solve_ivp not really batch-friendly
                y_new = []
                for i in range(batch):
                    sol = solve_ivp(self.du_dt, (t_step, t_step + self.step), 
                                    y_step[i], method=self.method, 
                                    t_eval=[t_step + self.step])
                    y_new.append(sol.y[:,-1])
                y_step = np.stack(y_new, axis=0)
            
            y_sol[:,t,:] = y_step
            t_step += self.step

        return y_sol
    

    def generate_ics(self, batch, y0 = None, sigma = 1, distribution = 'uniform', seed = None):
        np.random.seed(seed)

        if y0 is None:
            y0 = np.zeros(self.n_dim)
        y = np.array([y0 for _ in range(batch)])

        if distribution == "gaussian":
            y += np.random.normal(0, sigma, (batch, self.n_dim))
            
        elif distribution == "uniform":
            y += np.random.uniform(-sigma, sigma, (batch, self.n_dim))

        else:
            print('No such distribution')

        return y
    
lor_args = (10, 8/3, 28)

class lorenz(DS):
    def __init__(self, step = 0.02, params = lor_args, method = 'RK4'):
        
        def lorenz_du_dt(t, Z):
            Z = np.atleast_2d(Z) # ensures shape (batch, 3)
            u, v, w = Z[:, 0], Z[:, 1], Z[:, 2]# (batch, 1)
            sig, beta, rho = params
            
            up = -sig*(u - v)
            vp = rho*u - v - u*w
            wp = -beta*w + u*v
            
            return np.stack([up, vp, wp], axis = 1) # (batch, 3)
        
        super().__init__(du_dt = lorenz_du_dt,
                                 n_dim = 3,
                                 step = step,
                                 method = method)
    